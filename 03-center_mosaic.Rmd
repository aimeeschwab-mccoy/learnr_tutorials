...
The `favstats()` provides summary statistics about variables in data sets.

- What do you think `favstats()` stands for?

The code below calculates all summary statistics for the height of respondents in the`brfss` data set. Run this code, and use the output to answer the following questions.

```{r exercise1, exercise=TRUE, echo=TRUE}
library(mosaic)
favstats(~height, data=brfss)
```
...
```{r quiz2}
question("Identify the median height of respondents.",
  answer("67.2"),
  answer("4.13"),
  answer("48"),
  answer("64"),
...
Confirm the relationship between the mean and median desired weight.

```{r exercise3, exercise=TRUE, echo=TRUE}
favstats(~___, data=brfss)
```

```{r, exercise3-solution}
favstats(~wtdesire, data=brfss)
```

Based on the summary statistics, we can see that the mean for this variable is $\bar{x}=155$ and $m=150$. You may also notice that the maximum is quite large, $max=680$. 
...
We can find out by grouping our data with a vertical bar. In the code box below, group the data by exercise status, then calculate summary statistics for weight.

```{r exercise4, exercise=TRUE, echo=TRUE}
favstats(~___|___, data=___)
```

```{r, exercise4-solution}
favstats(~weight|exerany, data=brfss)
```
...
bwplot(~ratio|location, data=diabetes)
```
...
favstats(~ratio|location, data=diabetes)
```
