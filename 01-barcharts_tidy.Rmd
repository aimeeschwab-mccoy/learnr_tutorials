...
subtitle: "Learning Objective: Creating bar charts in the `tidyverse`"
...
```{r, echo=TRUE}
avandia %>% 
  group_by(treatment, cardiovascular_problems) %>%
  summarize(n=n())
```
...
Run the code in the box below to change the color of the bar for each treatment group.
...
avandia %>%
  ggplot(aes(x=cardiovascular_problems)) + 
  geom_bar(aes(fill=cardiovascular_problems)) + 
...
This plot is usually called a __stacked bar chart__, since the groups are "stacked" on one another.

Another option is a __separated bar chart__. In this case the bars are not stacked, but placed next to one another. Adding the `position='dodge'` option accomplishes this.
...
avandia %>%
  ggplot(aes(x=___)) + 
  geom_bar(aes(fill=___), position='dodge') + 
  labs(x='Treatment group', y='Count')
```
...
avandia %>%
  ggplot(aes(x=treatment)) + 
  geom_bar(aes(fill=cardiovascular_problems), position='dodge') + 
  labs(x='Treatment group', y='Count')
```
